...
const tableHeaders: TableItem[] = [
  { id: "1", title: "Experiência" },
  { id: "2", title: "Dados" },
];

const tableFields: TableItem[] = [
  { id: "1", title: "role" },
  { id: "2", title: "company" },
  { id: "3", title: "url" },
  { id: "4", title: "date" },
...
const newExperienceFields: FormField[] = [
  {
    id: "1",
    label: "role",
    placeholder: "Coloque a função que você exerceu na empresa",
  },
  { id: "2", label: "company", placeholder: "Coloque o nome da empresa" },
  { id: "3", label: "url", placeholder: "Coloque o url do deploy do projeto" },
  {
    id: "4",
    label: "date",
  ...
  { id: "6", placeholder: "Coloque a descrição sobre o projeto 2" },
  { id: "7", placeholder: "Coloque a descrição sobre o projeto 3" },
  {
    id: "8",
    label: "stacks",
...
const mockExperiences: ExperienceData[] = [
  ...
  },
  {
    ...
    details1:
      "Architected the front-end of a high-performance financial dashboard for cryptocurrency monitoring, building a reactive and scalable interface utilizing Next.js, React, and TypeScript.",
    details2:
      "Reduced initial load times by 70% and ensured the delivery of real-time market data by integrating the CoinGecko REST API coupled with Next.js Server-Side Rendering (SSR).",
    details3:
      "Enabled personalized asset management by engineering the 'My Portfolio' feature from scratch, guaranteeing mathematical precision and financial data integrity through strict TypeScript typing.",
    stacks: "React, Next.js, TypeScript, Tailwind CSS",
  },
  {
    id: "6",
    ...
    details1:
      "Optimized the conversion rate and new user onboarding flow, significantly reducing registration friction by implementing a robust social authentication system via OAuth integration (Google and GitHub).",
    details2:
      "Engineered the Single-Page Application (SPA) routing architecture, ensuring zero-latency screen transitions without page reloads by structuring the complete access flow with React Router DOM.",
    details3:
      "Ensured data integrity and reduced invalid server requests by implementing responsive forms with Tailwind CSS alongside strict client-side validation for real-time user feedback.",
  ...
  },
  {
    ...
    details1:
      "Ensured the stability of core features (authentication, transactions, and logout) and prevented production regressions by automating the end-to-end (E2E) user journey validation using Cypress.",
    details2:
      "Mitigated systemic flaws and UX issues in the access flow by implementing test coverage for edge cases (invalid data and blank fields) to strictly validate error handling in forms.",
    details3:
      "Ensured a consistent navigation experience across mobile devices by developing automated responsiveness and viewport transition tests to validate the correct rendering of menus and layouts.",
...
export default function FullContentPage() {
  const renderCellContent = (colTitle: string, experience: ExperienceData) => {
    switch (colTitle) {
      ...
      case "details":
        return (
          <div className="flex flex-col gap-4 py-2 leading-relaxed text-wrap">
      ...
      default:
        return experience[colTitle as keyof ExperienceData];
    }
  };
  ...
  return (
    <div className="p-6">
      ...
      <Card className="shadow-primary/50 border-muted shadow-lg">
        ...
        {/* --- Formulário para Criar Nova Experiência --- */}
        <CardContent className="flex flex-col gap-6 border-b pb-8">
          <div>
            <h2 className="text-xl font-semibold">Experiência</h2>
            <h3 className="border-b pb-2 text-lg">Crie uma nova experiência</h3>
          </div>

          <div className="flex flex-col">
            {newExperienceFields.map(({ id, label, placeholder }) => (
              <Field key={id} className="mb-4">
                <div className="flex w-full items-center gap-4">
                  <FieldLabel className="w-18.75 min-w-18.75 shrink-0 text-left font-medium capitalize">
                    {label}
                  </FieldLabel>
                  <Input placeholder={placeholder} className="w-full flex-1" />
                </div>
              </Field>
            ))}
          </div>

          <Button className="w-fit">Adicionar nova experiência</Button>
        </CardContent>

        {/* --- Listagem das Experiências Adicionadas --- */}
                ...
                <div className="border-muted mt-2 rounded-md border shadow-sm [&>div]:overflow-hidden">
                  <Table>
                    ...
                    <TableBody>
                      {tableFields.map((col) => (
                        <TableRow key={col.id}>
                          <TableCell className="w-37.5 pt-4 align-top font-medium capitalize">
                            {col.title}
                          </TableCell>

                          <TableCell className="wrap-break-word">
                            {renderCellContent(col.title, experience)}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
